...
if ([[ $TAG1 == "honey-cfg-$TAG" ]] || [[ $TAG2 == "honey-cfg-$TAG" ]]); then
  ...
  docker service update \
    --config-rm $TAG1 \
    --config-rm $TAG2 \
    --config-add source="honey-cfg-$TAG",target="/usr/src/app/honeypot/config/properties.cfg"\
    --config-add source="honey-data-$TAG",target="/usr/src/app/honeypot/data/senddata.json"\
    --with-registry-auth \
    replay-honeypot
fi
