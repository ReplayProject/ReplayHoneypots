<template>
    ...
    >
        <component-title>
            ...
            <template v-slot:pageName>Config Management</template>
        </component-title>
        <hr class="o-20" />
        <section class="ph2 pt2" v-if="canAddConfig">
            <div class="flex flex-auto justify-center pa2 mh2">
                <router-link
                    :to="'/configs/new'"
                    class="w-20 fr tc f4 link blue hover-dark-gray hover-shadow"
                    >Add New Honeypot Config</router-link
                >
            </div>
        </section>
...
export default {
    ...
    methods: {
        ...
        async loadItems() {
            ...
            //Get the configs
            try {
                const results = await api.getConfigs(undefined, sort, skip, undefined, limit)
                const total = await api.getConfigs(undefined, undefined, 0, ['_id'], undefined)
                ...
                for (let i = 0; i < results.data.docs.length; i++) {
                    const pageLink = '/configs/' + results.data.docs[i]._id
