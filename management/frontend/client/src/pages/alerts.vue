<template>
  <main
    class="w-100 ph3-m ph3-l"
    :class="{
      'w-75-m': $route.name != 'overview',
      'w-75-l': $route.name != 'overview'
    }"
  >
    <component-title>Alerts</component-title>
    <hr class="o-20" />
    <section class="ph2 pt2">
      <article class="mw8 center br2 ba b--light-blue bg-lightest-blue">
        <div class="dt-ns dt--fixed-ns w-100">
          <div class="pa3 pa4-ns dtc-ns v-mid">
            <div>
              <h2 class="fw4 blue mt0 mb3">Sample Alert</h2>
              <p class="black-70 measure lh-copy mv0">
                🤷 You hate to see it 🤷
              </p>
            </div>
          </div>
          <div class="pa3 pa4-ns dtc-ns v-mid">
            <a
              href="https://www.youtube.com/watch?v=oHg5SJYRHA0"
              class="no-underline f6 tc db w-100 pv3 bg-animate bg-blue hover-bg-dark-blue white br2"
              >Fix it!</a
            >
          </div>
        </div>
      </article>

      <!-- Choices -->
      <div class="flex flex-auto justify-center">
        <div
          v-for="n in [10, 20, 100, -1]"
          :key="n"
          @click="$toasted.show('Fix me Zach! Num: ' + n)"
          class="pointer b mv3 br2 ph4 pv2 mh2 ba b--blue blue hover-bg-blue hover-white shadow-hover"
        >
          {{ n > 0 ? 'Last ' + n : 'All' }}
        </div>
      </div>
    </section>

    <p class="ma3 pa2 i tc w-100">
      click on the "{...}" to expand the log and click on the references to open
      them in a new tab
    </p>

    <!-- List of alerts -->
    <ul class="list pl0 mt0 measure center">
      <li
        v-for="(a, idx) in alertsRaw.rows"
        :key="idx"
        class="alert cf items-center lh-copy pa3 ph0-l bb b--black-20"
      >
        <!-- container for icon and desc -->
        <div class="w-80 flex fl">
          <img class="w2 h2 w3-ns h3-ns br2" :src="variantMap[a.doc.variant]" />
          <div class="w-auto pl3">
            <span class="f6 db black">{{ a.doc.message }}</span>
            <span class="f6 db black-60 i">{{
              $parseDateWithTime(a.doc.timestamp)
            }}</span>
          </div>
        </div>
        <!-- Link / Hostname -->
        <router-link
          :to="'/details/' + a.doc.hostname"
          class="w-20 fr tr f4 link blue hover-dark-gray hover-shadow"
          >{{ a.doc.hostname }}</router-link
        >
        <!-- Pretty JSON -->
        <vue-json-pretty
          class="fl mt2 pa3 w-100 shadow-hover"
          style="background:#f4f4f470;"
          showLength
          showLine
          highlightMouseoverNode
          :showSelectController="false"
          :showDoubleQuotes="false"
          :deep="0"
          :path="'doc'"
          :data="a.doc"
          @click="jsonClick"
        />
      </li>
    </ul>
  </main>
</template>

<script>
import componentTitle from '../components/title'
import VueJsonPretty from 'vue-json-pretty'

export default {
  name: 'alerts',
  components: {
    componentTitle,
    VueJsonPretty
  },
  methods: {
    jsonClick (path, data) {
      // console.log(path, data)
      if (path.includes('.references[') && data.length != 0) {
        console.log('User clicked on a valid reference')
        window.open(this.dbURI + '/' + data)
      }
    }
  },
  data () {
    return {
      variantMap: {
        admin:
          'data:image/png;base64,',
        meta:
          'data:image/png;base64,',
        alert:
          'data:image/png;base64,'
      },
      alertsRaw: {
        total_rows: 8,
        offset: 0,
        rows: [
          {
            id: 'f65f84db3a0590346317c1b1a40004d3',
            key: 'f65f84db3a0590346317c1b1a40004d3',
            value: { rev: '1-cff864edb0767a6e89fd801b70d9aa89' },
            doc: {
              _id: 'f65f84db3a0590346317c1b1a40004d3',
              _rev: '1-cff864edb0767a6e89fd801b70d9aa89',
              variant: 'meta',
              message: 'Honeypot startup.',
              hostname: 'yogi',
              timestamp: 1587056722,
              references: [
                'f65f84db3a0590346317c1b1a40004fa',
                'f65f84db3a0590346317c1b1a40063f5'
              ]
            }
          },
          {
            id: 'f65f84db3a0590346317c1b1a400c7ea',
            key: 'f65f84db3a0590346317c1b1a400c7ea',
            value: { rev: '1-05c5fc9726fe5036585fcd3d967c77d2' },
            doc: {
              _id: 'f65f84db3a0590346317c1b1a400c7ea',
              _rev: '1-05c5fc9726fe5036585fcd3d967c77d2',
              variant: 'alert',
              message: 'Incoming Scan Detected from: 192.168.1.68',
              hostname: 'yogi',
              timestamp: 1587056766,
              references: []
            }
          },
          {
            id: 'f65f84db3a0590346317c1b1a402fd54',
            key: 'f65f84db3a0590346317c1b1a402fd54',
            value: { rev: '1-3ed1cdc731d3f8ad5e97b603965f0e91' },
            doc: {
              _id: 'f65f84db3a0590346317c1b1a402fd54',
              _rev: '1-3ed1cdc731d3f8ad5e97b603965f0e91',
              variant: 'admin',
              message: 'Configuration Update from: "John"',
              hostname: 'yogi',
              timestamp: 1587056894,
              references: []
            }
          },
          {
            id: 'f65f84db3a0590346317c1b1a4036226',
            key: 'f65f84db3a0590346317c1b1a4036226',
            value: { rev: '1-ae1f5fb8fbcf6631589e4f1aac152e6c' },
            doc: {
              _id: 'f65f84db3a0590346317c1b1a4036226',
              _rev: '1-ae1f5fb8fbcf6631589e4f1aac152e6c',
              variant: 'meta',
              message: 'Honeypot startup.',
              hostname: 'winnie',
              timestamp: 1587056905,
              references: []
            }
          },
          {
            id: 'f65f84db3a0590346317c1b1a40367c3',
            key: 'f65f84db3a0590346317c1b1a40367c3',
            value: { rev: '1-ae1f5fb8fbcf6631589e4f1aac152e6c' },
            doc: {
              _id: 'f65f84db3a0590346317c1b1a40367c3',
              _rev: '1-ae1f5fb8fbcf6631589e4f1aac152e6c',
              variant: 'meta',
              message: 'Honeypot startup.',
              hostname: 'sd-vm06',
              timestamp: 1587056905,
              references: []
            }
          },
          {
            id: 'f65f84db3a0590346317c1b1a403f363',
            key: 'f65f84db3a0590346317c1b1a403f363',
            value: { rev: '1-1ebfe5a4ac066d0a44fa98001699d832' },
            doc: {
              _id: 'f65f84db3a0590346317c1b1a403f363',
              _rev: '1-1ebfe5a4ac066d0a44fa98001699d832',
              variant: 'admin',
              message: 'Configuration update from: "Seth".',
              hostname: 'yogi',
              timestamp: 1587056922,
              references: []
            }
          },
          {
            id: 'f65f84db3a0590346317c1b1a407d265',
            key: 'f65f84db3a0590346317c1b1a407d265',
            value: { rev: '1-edda6499730dbfb145eb2b3d6de813b3' },
            doc: {
              _id: 'f65f84db3a0590346317c1b1a407d265',
              _rev: '1-edda6499730dbfb145eb2b3d6de813b3',
              variant: 'meta',
              message:
                'This is what a really really long message that would be sent for an alert would look like, please help this honeypot is just begging for attention with this really long message.',
              hostname: 'yogi',
              timestamp: 1587057604,
              references: []
            }
          },
          {
            id: 'f65f84db3a0590346317c1b1a407db69',
            key: 'f65f84db3a0590346317c1b1a407db69',
            value: { rev: '1-8cda51bf09439f90688464fb385d5dc4' },
            doc: {
              _id: 'f65f84db3a0590346317c1b1a407db69',
              _rev: '1-8cda51bf09439f90688464fb385d5dc4',
              variant: 'meta',
              message: 'Honeypot startup.',
              hostname: 'winnie',
              timestamp: 1587057845,
              references: []
            }
          }
        ]
      }
    }
  }
}
</script>
