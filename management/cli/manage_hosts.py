from utilities import log, style, hostselector, setupConfig, writeConfig, hosts, hostdata, DeviceIPValidator, EmptyValidator, FilePathValidator, HostnameValidator, PortValidator
...
def checkstatus(ctx, selected_hosts=None):
    ...
    for host in selected_hosts:
        if host in all_hosts:
            ...
            stdout, stderr = subprocess.Popen(['ssh', '-i', ssh_key, (user + '@' + ip),'-p', ssh_port, 'uname -a'],
